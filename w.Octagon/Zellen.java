...
public class Zellen {
    ...
    public static boolean[][] NeueGeneration (boolean[][] A) {
        ...
        for (int i = 0; i <= B.length-1; i++) {
            for (int j = 0; j <= B[0].length-1; j++) {
                int[] step = {i+1,j+1}; // in mathmatical logic, too of course
    ...
    public static void matrixAusgeben (boolean[][] A) { // copied and modified from last exercise-sheet
        for (int i = 0; i <= A.length-1; i++) { // rows
            for (int j = 0; j <= A[0].length-1; j++) { // columns
                if ( j < A[0].length-1) {
                    if ( A[i][j] ) {
                        System.out.print("*");
                    }
    ...
    public static void main (String[] args) throws InterruptedException {
        ...
        matrixAusgeben(A);
        System.out.print("Do you want to see the 23/3-Octagon-Developement? ");
        String dev = sc.next();
        if ( dev.equals("y") ) {
            boolean[][] Oktagon = {{false, false, false, false, false, false, false, false, false, false},
                                   ...
                                   {false, false, false, false, false, false, false, false, false, false},
                                   {false, false, false, false, false, false, false, false, false, false}};
            matrixAusgeben(Oktagon);
            while ( !compare(Oktagon,NeueGeneration(Oktagon)) ) {
                Oktagon = NeueGeneration(Oktagon);
                matrixAusgeben(Oktagon);
                Thread.sleep(300);
                clearScreen();
            }
